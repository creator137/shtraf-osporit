...
PERSONAL_DATA_CONSENT_VERSION = "pd-consent-v3"
...
PERSONAL_DATA_CONSENT_TEXT = (
    ...
    "Перед загрузкой по возможности закройте или удалите из документа персональные "
    "данные, которые не нужны для проверки штрафа. Если вы оставляете такие данные "
    "и нажимаете «Согласен», вы разрешаете их обработку для указанных ниже целей.\n\n"
    "Цель обработки: создание и ведение дела по оспариванию штрафа, хранение "
    "документов и просмотр дела оператором сервиса.\n\n"
    "Для распознавания постановление может передаваться внешнему OCR-провайдеру "
    "OCR.space. Для юридического анализа распознанный текст и данные дела могут "
    "передаваться сервису DeepSeek.\n\n"
    "Telegram используется как интерфейс. Данные дела и документы сохраняются "
    "в серверной инфраструктуре сервиса. Удалить профиль, согласие, дела и файлы "
    "из системы можно командой /restart."
)
